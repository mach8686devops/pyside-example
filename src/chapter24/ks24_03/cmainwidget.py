...
from PyQt5.QtCore import QPointF, QPropertyAnimation, QEasingCurve, QPoint, QByteArray
from cprojects import *
from cprojectinfo import *
from cburndown import *
from config import *
from cuser import *
from csprint import *
from caddsprint import *
from clogindialog import *
...
class CMainWidget(QWidget, Ui_CMainWidget):
	...
	def slot_menu(self):
		...
		if self.m_bShowMenu:  # 单击时弹出菜单
			...
			self.m_pAnimaMenuShow.setStartValue(QPoint(self.x() + self.width(), self.y() + offsetY))
			self.m_pAnimaMenuShow.setEndValue(QPoint(self.x() + self.width() - s_MenuWidth, self.y() + offsetY))
			self.m_pAnimaMenuShow.start()
		
		else:  # 再次单击时隐藏菜单
			offsetY = s_MenubarHeight
			self.m_pAnimaMenuShow.setStartValue(QPoint(self.x() + self.width() - s_MenuWidth, self.y() + offsetY))
	...
	def initialize_database(self):
		...
		# 创建项目表
		sql_query = QSqlQuery()
		if not sql_query.exec("create table if not exists projects(id integer primary key autoincrement, name text unique, code text unique,startdate text, enddate text, sprintcycle int default 1, workloade  integer default 0,workloadfinished integer default 1, po text, sm text, tester text,currentsprint integer, devteam text)"):
			print(sql_query.lastError().text())
		# 创建人员表
		if not sql_query.exec("create table if not exists users(id integer primary key autoincrement, name text unique, password text, role int, photo blob)"):
			print(sql_query.lastError().text())
		# 创建迭代表
		if not sql_query.exec("create table if not exists sprints( id integer primary key autoincrement, projectcode text, startdate text, enddate text, workloade integer, completion_date text)"):
			print(sql_query.lastError().text())		
		# 创建燃尽图表
		if not sql_query.exec("create table if not exists burndown(projectcode text, sprintid integer, date text, workload integer)"):
			print(sql_query.lastError().text())		
		# 创建待办事项表
		if not sql_query.exec("create table if not exists backlogs(id integer primary key autoincrement, projectcode text, sprintid integer, backlog text, workloade int, workloadr integer, state integer, info text)"):
			print(sql_query.lastError().text())		
		# 创建分解任务表
		if not sql_query.exec("create table if not exists tasks(id integer primary key autoincrement, task text default \'\', projectcode text,  backlogid integer, developer text, workloade integer, workloadr integer, state integer default 0)"):
			print(sql_query.lastError().text())
