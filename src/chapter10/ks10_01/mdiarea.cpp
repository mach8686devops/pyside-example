...
CEditMdiArea::CEditMdiArea(QMainWindow* pMainWindow) : 
	QMdiArea(pMainWindow),
...
CGraphView* CEditMdiArea::createMdiChild(const QString& fileName, QString* /*pError*/) {
	CGraphView* pView = new CGraphView(this, fileName);
...
void CEditMdiArea::onViewClose(QWidget* pChild) {
	...
	if (NULL != pView) {
		disconnectEditViewWithSlot(pView); // 将编辑视图与多窗口区域的槽函数断开关联
	}
	m_pLastActivatedMdiChild = NULL;
	emit editViewClose(pChild);
	m_pLastActivatedMdiChild = NULL;	// 防止其他对象对于 editViewClose 信号的处理导致触发activeSubWindow信号
										// 从而使m_pLastActivatedMdiChild重新被赋值（为已关闭的窗口）
}
void CEditMdiArea::connectEditViewWithSlot(CGraphView* pView) {
    connect(pView, SIGNAL(viewMouseMove(const QPointF&)),
        this, SIGNAL(viewMouseMove(const QPointF&)));
