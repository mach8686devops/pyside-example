...
void CGraphView::calculateSceneRect() {
	QGraphicsScene* pScene = scene();
	QList<QGraphicsItem*> lst = pScene->items();
	QRectF rectScene = pScene->sceneRect();
	QRectF itemRect;
	QList<QGraphicsItem*>::iterator iteLst = lst.begin();
	for (; iteLst != lst.end(); iteLst++) {
		itemRect = (*iteLst)->boundingRect(); // 得到图元的外接矩形（图元坐标系）
		itemRect = (*iteLst)->mapToScene(itemRect).boundingRect();// 将图元的外接矩形转换到场景坐标系
		/* 根据所有图元的外接矩形计算场景的范围 */
