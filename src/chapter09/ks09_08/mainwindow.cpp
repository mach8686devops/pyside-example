...
void CMainWindow::open() {
	...
	QString szDir = ns_train::getDirectory("$TRAINDEVHOME/test/chapter09/ks09_08/"); // 获取指定目录的字符串
	QString strFilter = "text file(*.txt);;XML file(*.xml);;all file(*.*)"; // 过滤器之间用两个分号进行分隔
#ifdef WIN32
	...
	QString fileName = QFileDialog::getOpenFileName(this,
													...
													strFilter,
													NULL,
													QFileDialog::DontUseNativeDialog); // 在Linux上不再使用本机的打开文件对话框,保持与Windows风格一致

#endif
	if (!fileName.isEmpty()) {
		fileName = ns_train::getFileName(fileName);
		QString strCurrentFileName = m_pTextEdit->currentFileName();
		if (strCurrentFileName == fileName) { // 已经打开过
			return;
		}
