...
CMainWindow::CMainWindow(QWidget* parent) : QMainWindow(parent), m_pInfoLabel(NULL), m_pMouseLabel(NULL), m_pTextEdit(NULL) {
	createActions();
	createMenus();
	createToolBars();
	createStatusBar();
...
void CMainWindow::onSelectionChanged() {
	QTextCursor tc = m_pTextEdit->textCursor(); // 获取光标下的文本对象
	QTextCharFormat textCharFormat = tc.charFormat(); // 获取它的格式
	bool b = textCharFormat.font().bold(); // 获取字体的粗体信息	
	m_pBoldAct->blockSignals(true);	// 阻塞信号，防止changed()信号触发槽函数。triggered信号无需调用blockSignals().
	m_pBoldAct->setChecked(b);		// 设置粗体按钮的状态
	m_pBoldAct->blockSignals(false);// 解除阻塞
}
...
void CMainWindow::bold() {
	...
	bool bChecked = m_pBoldAct->isChecked();
...
void CMainWindow::createActions() {
	...
	//connect(m_pBoldAct, &QAction::changed, this, &CMainWindow::bold);
