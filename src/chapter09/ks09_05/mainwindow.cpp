...
void CMainWindow::initialize() {
	...
	//connect(m_pTextEdit, 
	//		SIGNAL(viewMouseMove(QMouseEvent*)), 
	//		this, 
	//		SLOT(onMouseMoveInView(QMouseEvent*)));
	connect(m_pTextEdit, &CTextEdit::viewMouseMove,
			this, &CMainWindow::onMouseMoveInView);
...
void CMainWindow::createStatusBar() {
	m_pInfoLabel = new QLabel(tr(""), this);
	...
	m_pMouseLabel = new QLabel(tr(""), this);
	m_pMouseLabel->setMinimumWidth(100); // 防止太窄导致无法完整显示
...
void CMainWindow::onMouseMoveInView(QMouseEvent* e) {
	// todo
	QPointF ptLocal = e->localPos();
	QPoint pt = ptLocal.toPoint();
	QString strText;
	strText.sprintf("%d, %d", pt.x(), pt.y());
	m_pMouseLabel->setText(strText);
}