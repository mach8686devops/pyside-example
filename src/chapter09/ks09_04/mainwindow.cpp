...
CMainWindow::CMainWindow(QWidget* parent) : QMainWindow(parent) {
	...
	m_pInfoLabel->setAlignment(Qt::AlignCenter);	// 设置对齐方式
	statusBar()->addPermanentWidget(m_pInfoLabel);	// 添加到状态条
...
void CMainWindow::createMenus() {
	...
	m_pFileMenu->addAction(m_pOpenAct); // 将菜单项添加到菜单
	m_pFileMenu->addAction(m_pOpenAct);
	m_pFileMenu->addAction(m_pSaveAct);
	m_pFileMenu->addSeparator(); // 添加分隔线
