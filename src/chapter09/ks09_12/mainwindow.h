...
private:
	...
	void createDockWindows();	/// 构建浮动窗
	void createDockWindows_Left();	/// 构建左侧的浮动窗
	void createDockWindows_Down();	/// 构建下部的浮动窗

	void connectSignalAndSlot();/// 绑定信号槽
	void notify(const SLog&);	/// 封装好的日志通知接口。用来把日志通知到日志窗。
	CCountry* createCountry(void); /// 构建CCountry对象.
...
private:
	...
	#QMutex		m_mtx;			// 互斥锁，用来保护对m_pLogDockWidget的访问
	CLogDockWidget*	m_pLogDockWidget; /// 底部的浮动窗，用来展示日志
};
