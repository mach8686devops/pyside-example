...
void CGraphView::mouseMoveEvent(QMouseEvent *e) {
    ...
    emit(viewMouseMove(pt)); // 发射信号，以便可以在状态栏显示鼠标坐标
}
...
void CGraphView::cut() {
	...
	if (lst.size() == 0)
		return;
	// 剪切时采用隐藏图元、再拷贝得到新图元的方法，也可以拷贝得到新图元后把原始图元删除。但是如果删除原始图元，则会影响redo/undo功能。
...
void CGraphView::copyItems(QList<QGraphicsItem*>& lst) {
	...
	stream << document; // 把XML内容序列化到stream对象中

	QMimeData* data = new QMimeData();
	data->setData("gp/copyItem", dataArr); // gp/copyItem用来区分剪切板中的不同内容
	clipboard->setMimeData(data); // 把数据放到剪切板
}

void CGraphView::paste() {
	QClipboard* clipboard = QApplication::clipboard();
	const QMimeData* mimeData = clipboard->mimeData(); // 从剪切板中得到待粘贴的数据
	QGraphicsScene* pScene = scene();
	if (NULL == pScene)
		return;
	if (mimeData->hasFormat("gp/copyItem")) { // 是否存在期望的数据
		QDomDocument doc;
