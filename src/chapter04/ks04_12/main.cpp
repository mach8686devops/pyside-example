...
{
    QString strName;
    strName = ns_train::getPath("$TRAINDEVHOME/test/chapter04/ks04_12/example01.txt");
	QString strDir = ns_train::getDirectory(strName);
	QDir dir;
	dir.mkpath(strDir);
    ...
    if (!file.open(QFile::Truncate | QFile::WriteOnly | QFile::Text)) {
        qDebug("open failed! file name is:%s", strName.toLocal8Bit().data());
        return;
    }
    // 输出整个文件的内容
    QString strContent;
