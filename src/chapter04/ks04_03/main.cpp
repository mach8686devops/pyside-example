...
int main(int argc, char * argv[]) {
	...
	strPath += "/system/lang";					// $TRAINDEVHOME/system/lang/ks04_03.qm
	...
	if (0) {
		...
		cout << strInput.toLocal8Bit().data() << endl;
		cout << strDir.toLocal8Bit().data() << endl;
		cout << strFileName.toLocal8Bit().data() << endl;
	}

	// example02, 组织日志信息
	if (0) {
		QString strWholeInfo = example02(3, "Lisa", "adm01", "modified data at 12:00PM.");
		cout << strWholeInfo.toLocal8Bit().data();
	}

	// example03, 使用arg()接口对信息格式化, 输出矩形的坐标、宽高
	if (0) {
		QRectF rct(20.f, 30.f, 50.f, 60.f);
		QString str = example03(rct);
...
void example05(){
	...
	// 删除首尾多余的空格
	str = str.trimmed();	
	//cout << "str:" << str.toLocal8Bit().data() << endl;
...
void example06() {
	...
	cout << "int32 data = " << nInt32 << endl;	// 输出: int32 data = 0(原因，溢出。)
	cout << "uInt32 data = " << uInt32 << endl;	// 输出: uInt32 data = 2147483648

	QString str2;
	nInt32 = 2147483647;
	str1.sprintf("%d", nInt32);
	cout << "new int32 data = " << str1.toLocal8Bit().data() << endl;	// 输出: new int32 data = 2147483647

	str2.sprintf("%d", uInt32);
	cout << "new uint32 data = " << str2.toLocal8Bit().data() << endl;	// 输出: new uint32 data = -2147483648 

	str2.sprintf("%u", uInt32);
	cout << "new uint32 data = " << str2.toLocal8Bit().data() << endl;	// 输出: new uint32 data = 2147483648

	str1 = QString("%1").arg(nInt32);
	cout << "another new int32 data = " << str1.toLocal8Bit().data() << endl; // 输出: another new int32 data = 2147483647

	QString str3 = "200.f";
	float f = str3.toFloat();
	cout << "float data = " << f << endl;	// 输出: float data = 0(原因，不应该带末尾的'f'字符。)

	str3 = "200";
	f = str3.toFloat();
	cout << "float data = " << f << endl;	// 输出: float data = 200

	QString str4 = "3.1415926535897932";
	qreal d = str4.toDouble();
	QString strData;
	strData = QString::number(d);
	cout << "double data = " << strData.toLocal8Bit().data() << endl;	// 输出: double data = 3.14159
	strData = QString::number(d, 'g', 16); // 需要明确指明精度，否则默认只有6位小数。
	cout << "double data = " << strData.toLocal8Bit().data() << endl;	// 输出: double data = 3.141592653589793
