# -*- coding: utf-8 -*-
from PyQt5.QtWidgets import QApplication, QMainWindow, QAction, QActionGroup, QMenu, QLabel, QFrame, QMessageBox, QTreeView, QDockWidget
from PyQt5.QtGui import QIcon, QKeySequence, QStandardItemModel, QStandardItem, QColor, QFont, QImage
from PyQt5.QtCore import Qt, QFile, QTextStream, pyqtSignal, QThread, QSize, QMutex, QMutexLocker,QTime
from textedit import CTextEdit
import os
from splashscreen import CSplashScreen
from logdockwidget import CLogDockWidget
from logevt import CLogEvt, ELogLevel, SLog
...
class CMainWindow(QMainWindow):	
	...
	def openFile(self, strTitle) :
		...
		if strTitle.strip():
			...
			if None is trainDevHome:
				trainDevHome = 'usr/local/gui'
			fileName = trainDevHome + '/test/chapter19/ks19_10/'
			fileName += strTitle
			fileName += '.txt'
			str = self.textEdit.currentFileName()
			if (str == fileName) :
				return
